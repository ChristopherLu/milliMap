...
Pytorch implementation of our method for 2D dense mapping via a low-cost, off-the-shelf mmWave radar ([TI AWR1443](https://www.ti.com/product/AWR1443)). Our method can reconstruct a dense grid map with accuracy comparable to a lidar. It can be used for turning semantic label maps into photo-realistic images or synthesizing portraits from face label maps. <br><br>
...
### Testing
- Please download the pre-trained milliMap model from [here](https://www.dropbox.com/s/nfwq1cjcnznadzc/best_net_G.pth?dl=0) (dropbox link), and put it under `./checkpoints/millimap/`
...
```
The test results will be summarized to a html file here: `./results/millimap/test_best/index.html`, with generated images in the same level of folder.

More example scripts can be found in the `scripts` directory.

### Training
- Train a model (`bash ./scripts/train_millimap.sh`):
...
python train.py --name full_line_10 \
    ...
    --tf_log
```
- To view training results, please checkout intermediate results in `./checkpoints/millimap/web/index.html`.
- If you have tensorflow installed, you can see tensorboard logs in `./checkpoints/millimap/logs` by adding `--tf_log` to the training scripts.
...
When training on customized dataset is needed, please change the dataroot filed in the training scripts accordingly. Please refer to [pix2pixHD](https://github.com/NVIDIA/pix2pixHD) first to see how to prepare a dataset in valid format.
